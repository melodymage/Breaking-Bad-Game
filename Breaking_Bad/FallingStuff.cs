...
namespace Breaking_Bad
{
    class FallingStuff
    {
        ...
        public void moneyfly(GameTime gameTime)
        {
            if(position.X >= 0)
                position.X -= speed;
        }
        public FallingStuff(Texture2D texture, GraphicsDeviceManager graphics)
        {
            player = texture;
            scale = 1.0f;
        ...
        public void draw(SpriteBatch spriteBatch)
        {
            ...
            if (visible)
                spriteBatch.Draw(player, position, null, Color.White, rotation, center, scale, spriteEffects, layer);
        }
